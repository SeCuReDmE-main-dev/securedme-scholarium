...
type PayPalConfig = { clientId: string; clientSecret: string; mode: "live" | "sandbox"; redirectUri: string; sessionSecret: string };
...
async function config(): Promise<PayPalConfig | null> { const [clientId, clientSecret, redirectUri, sessionSecret, requestedMode] = await Promise.all([value("PAYPAL_LOGIN_CLIENT_ID"), value("PAYPAL_LOGIN_CLIENT_SECRET"), value("PAYPAL_LOGIN_REDIRECT_URI"), getScholariumSessionSecret(), value("PAYPAL_LOGIN_MODE")]); const mode = requestedMode === "live" || requestedMode === "sandbox" ? requestedMode : null; return clientId && clientSecret && redirectUri && sessionSecret && mode ? { clientId, clientSecret, mode, redirectUri, sessionSecret } : null; }
function endpoint(settings: PayPalConfig, path: string) { return `${settings.mode === "live" ? "https://www.paypal.com" : "https://www.sandbox.paypal.com"}${path}`; }
function api(settings: PayPalConfig, path: string) { return `${settings.mode === "live" ? "https://api-m.paypal.com" : "https://api-m.sandbox.paypal.com"}${path}`; }
...
export async function paypalStartResponse(request: Request) {
  ...
  const authorizationUrl = new URL(endpoint(settings, "/connect"));
...
export async function paypalCallbackResponse(request: Request) {
  ...
  const tokenResponse = await fetch(api(settings, "/v1/oauth2/token"), { body: new URLSearchParams({ code, grant_type: "authorization_code", redirect_uri: settings.redirectUri }), headers: { authorization, "content-type": "application/x-www-form-urlencoded" }, method: "POST" });
  ...
  const profileResponse = await fetch(`${api(settings, "/v1/identity/oauth2/userinfo")}?schema=paypalv1.1`, { headers: { authorization: `Bearer ${token.access_token}` } });
