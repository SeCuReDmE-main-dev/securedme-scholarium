const contract = {
  ...
  paths: {
    ...
    "/api/v1/teach/engine/session": { post: { summary: "Open or resume a D1-canonical immutable syllabic-engine session" } },
    "/api/v1/teach/engine/card": { get: { summary: "Read the active syllable card and two preloaded successors without advancing progression" } },
    "/api/v1/teach/engine/attempt": { post: { summary: "Submit an idempotent bounded attempt and atomically persist its deterministic receipt and checkpoint" } },
    "/api/v1/teach/engine/progress": { get: { summary: "Read the D1-canonical syllabic checkpoint projection" } },
    "/api/v1/teach/engine/audio-observation": { post: { summary: "Forward ephemeral adult-consented audio for non-authoritative observation; minors remain blocked" } },
    ...
    "/api/v1/teach/exceptional-access": { get: { summary: "List organization-scoped exceptional-access requests without learner content" }, post: { summary: "Create a justified exceptional-access request expiring within four hours" }, put: { summary: "Consume one dual-approved request exactly once and receive a bounded projection" } },
    "/api/v1/teach/exceptional-access/decisions": { post: { summary: "Approve or deny an exceptional-access request as an independent authorized administrator" } },
    "/api/v1/teach/safety-cases": {
      get: { summary: "List reporter-owned or tenant-authorized synthetic school safety cases without private evidence" },
      post: { summary: "Create an idempotent synthetic school safety case for human review under an active school policy" },
    },
